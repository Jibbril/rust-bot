...
mod tests {
    ...
    use crate::{models::{candle::Candle, timeseries::TimeSeries}, indicators::{populates_candles::PopulatesCandles, indicator_type::IndicatorType}};
    use super::BBWP;
    ...
    fn calculate_bbwp() {
        ...
        for (i,val) in correct_values.iter().enumerate() {
            let bbwp = segment[i].get_indicator(&IndicatorType::BBWP(13, 252)).unwrap().as_bbwp().unwrap();
    ...
    fn bbwp_not_enough_data() {
        ...
        for candle in ts.candles.iter() {
            let bbwp = candle.indicators.get(&IndicatorType::BBWP(13, 252)).unwrap().as_bbwp();
